#####################################################################
# Knersey-ney Optimazed VFinal 3
    ...
    if (file.exists(file.name.final)) {
      ...
      switch(n,
             "1" = { # For each unigram lets calculate the kneser-ney prob
               ...
               #  Pkn(t1) := N1+(* t1) / N1+(**) = N1+(* t1) / ???w N1+(* w)
               
               ##### To calculate this we need: 
               #       N1+(* t1) = n11(t1)       (to be calculated) 
               #       ???w N1+(* w) = n11.all     (to be calculated)
               ...
               DT.uni.prob <- merge(DT.uni.prob, DT.n12 , by = "t1")
               ...
               # Calculate sum.freq2, to be used on Bigrams Calculations
               print("... Calculating sum w c(t1 w) = sum.freq2(t1) ...")
               
               setkey(DT.uni.temp,"t1")
               setkey(DT.bi.temp,"t1")
               
               DT.sum.freq2 <- DT.bi.temp[DT.uni.temp, sum(freq2), by = .EACHI] [,c("t1","sum.freq2","V1"):=list(t1,V1,NULL)]
               DT.uni.prob <- merge(DT.uni.prob, DT.sum.freq2 , by = "t1")
               
               
               print("... Calculating Kneser-ney Prob for unigrams ...")
               print("...   Pkn(t1) := N1+(* t1) / N1+(**) = N1+(* t1) / ???w N1+(* w) ...")
               ...
               #       ???w c(w) = uni.all      (to be calculated)
               
               # Calculate MLE of Unigrams
               print("... Calculating MLE for Unigrams MLE(t1)= c(t1) / ???w c(w) ... ")
             ...
             "2" = {
               ...
               #  Pkn(t1 t2) := max{ c(t1 t2) - D2, 0 } / ???w c(t1 w) + D2 / ???w c(t1 w) * N1+(t1 *) x Pknr (t2)
               
               ##### To calculate this we need 
               #       c(t1 t2) = freq2(t1 t2)    (on Bigrams Prob Table) 
               #       ???w c(t1 w) = sum.freq2(t1) (on Unigram Prob Table)
               #       D2                         (already calculated on init)
               #       N1+(t1 *) = n12(t1)        (on Unigram Prob Table)
               #       Pknr(t2)  = pkn12(t2)      (on Unigram Prob Table)
               
              
               print("Calculating Kneser-Ney Prob for Bigrams ...")
               
               # Let's add sum.freq2(t1) and n12(t1) to the Bigrams Table
               
               DT.uni.temp1 <- copy(DT.uni.prob)
               DT.uni.temp1 <- DT.uni.temp1[,list(t1,sum.freq2,n12),]
               ...
               print("... Calculating Kneser-ney Prob for High Order Bigrams = pkn21(t1 t2)...")
               print("...   Pkn(t1 t2) := max{ c(t1 t2) - D2, 0 } / ???w c(t1 w) + ")
               print("                    D2 / ???w c(t1 w) * N1+(t1 *) x Pknr (t2) ...")
               ...
               #################################################################################################
               # Calculate Knersey Prob for Low Order Bigrams (to be used on Trigrams Calculations)
               #  Pknr(t1 t2) := max{ N1+(* t1 t2) - D2, 0 } / ???w N1+(* t1 w) + D2 / ???w N1+(* t1 w) * N1+(t1 *) x Pknr (t2) 
               
               ##### To calculate this we need 
               #       N1+(* t1 t2) = n21(t1 t2)    (to be calculated) 
               #       ???w N1+(* t1 w) = sum.n22(t1) (to be calculated)
               #       D2                           (already calculated on init)
               #       N1+(t1 *) = n12(t1)          (on Unigram Prob Table)
               #       Pknr(t2)  = pkn12(t2)        (on Unigram Prob Table)
               
               DT.bi.temp <- copy(DT.bi.prob)
               DT.tri.temp <- copy(DT.tri.prob)
               
               # Calculate n21 
               print("... Calculating N1+(* t1 t2) = n21(t1 t2) ...")
               ...
               DT.bi.prob <- merge(DT.bi.prob, DT.n21 , by = c("t1","t2"))
               ...
               DT.bi.prob <- merge(DT.bi.prob, DT.n22 , by = c("t1","t2"))
               
               # Calculate sum.n22
               print("... Calculating ???w N1+(* t1 w) = sum.n22(t1) ...")
               ...
               DT.sum.n22 <- DT.bi.temp[DT.uni.temp, sum(n22), by = c("t1")] [,c("sum.n22","V1"):=list(V1,NULL)]

               DT.bi.prob <- merge(DT.bi.prob, DT.sum.n22 , by = "t1")
               
               print("... Calculating Kneser-ney Prob for low Order Bigrams = pkn22(t1 t2)...")
               print("...   Pknr(t1 t2) := max{ N1+(t1 t2) - D2, 0 } / ???w N1+(* t1 w) + ")
               print("                     D2 / ???w N1+(* t1 w) * N1+(t1 *) x Pknr (t2) ...")
               
               
               DT.bi.prob[, pkn22:= 
                            ifelse(D2 > n21,0,n21 - D2) /  sum.n22 + 
                            D2 / sum.n22 * n12 * # lambda
                            pkn12,]
               
               
               # Calculate sum.freq3, to be used on Trigrams Calculations
               print("... Calculating ???w c(t1 t2 w) = sum.freq3(t1 t2) ...")
               
               setkeyv(DT.bi.temp,c("t1","t2"))
               setkeyv(DT.tri.temp,c("t1","t2"))
               
               DT.sum.freq3 <- DT.tri.temp[DT.bi.temp, sum(freq3), by = .EACHI] [,c("t1","t2","sum.freq3","V1"):=list(t1,t2,V1,NULL)]
               DT.bi.prob <- merge(DT.bi.prob, DT.sum.freq3 , by = c("t1","t2"))
             ...
             "3" = {
               ...
               #  Pkn(t1 t2 t3) := max{ c(t1 t2 t3) - D3, 0 } / ???w c(t1 t2 w) + D3 / ???w c(t1 t2 w) * N1+(t1 t2 *) x Pknr (t2 t3)
               
               ##### To calculate this we need 
               #       c(t1 t2 t3) = freq3(t1 t2 t3)        (on Trigrams Prob Table) 
               #       ???w c(t1 t2 w) = sum.freq3(t1 t2)     (on Bigrams Prob Table)
               #       D3                                   (already calculated on init)
               #       N1+(t1 t2 *) = n22(t1 t2)            (on Bigrams Prop Table)
               #       Pknr(t2 t3)  = pkn22(t2 t3)          (on Bigrams Prob Table)
               
               print("Calculating Knersey-ney prob for Trigrams ...")
               
               
               
               # Let's add sum.freq3(t1 t2) and n22(t1 t2) to the Trigrams Table
               
               DT.bi.temp1 <- copy(DT.bi.prob)
               DT.bi.temp1 <- DT.bi.temp1[,list(t1,t2,sum.freq3,n22),]
               DT.tri.prob <- merge(DT.tri.prob, DT.bi.temp1 , by = c("t1","t2"))
               
               # Let's add pkn22(t2 t3) to the Trigrams Table
               
               DT.bi.temp2 <- copy(DT.bi.prob)
               DT.bi.temp2[,c("t3","t2") := list(t2,t1),] 
               DT.bi.temp2 <- DT.bi.temp2[,list(t2,t3,pkn22),]
               DT.tri.prob <- merge(DT.tri.prob, DT.bi.temp2 , by = c("t2","t3"))
               ...
               #################################################################################################
               # Calculate Knersey Prob for Low Order Trigrams (to be used on Quadgrams Calculations)
               #  Pknr(t1 t2 t3) := max{ N1+(* t1 t2 t3) - D3, 0 } / ???w N1+(* t1 t2 w) + D3 / ???w N1+(* t1 t2 w) * N1+(t1 t2 *) x Pknr (t2 t3) 
               
               ##### To calculate this we need 
               #       N1+(* t1 t2 t3) = n31(t1 t2 t3)    (to be calculated) 
               #       ???w N1+(* t1 t2 w) = sum.n32(t1 t2) (to be calculated)
               #       D3                                 (already calculated on init)
               #       N1+(t1 t2 *) = n22(t1 t2)          (on Bigram Prob Table)
               #       Pknr(t2 t3)  = pkn22(t2 t3)        (on Bigram Prob Table)
               
               DT.tri.temp <- copy(DT.tri.prob)
               DT.quad.temp <- copy(DT.quad.prob)
               
               # Calculate n31 
               print("... Calculating N1+(* t1 t2 t3) = n31(t1 t2 t3) ...")
               ...
               DT.tri.prob <- merge(DT.tri.prob, DT.n31 , by = c("t1","t2","t3"))
               DT.tri.prob <- merge(DT.tri.prob, DT.n32 , by = c("t1","t2","t3"))
               

               # Calculate sum.n32
               print("... Calculating ???w N1+(* t1 t2 w) = sum.n32(t1 t2) ...")
               ...
               DT.sum.n32 <- DT.tri.temp[DT.bi.temp, sum(n32), by = c("t1","t2")] [,c("sum.n32","V1"):=list(V1,NULL)]
               DT.tri.prob <- merge(DT.tri.prob, DT.sum.n32 , by = c("t1","t2"))
               
               print("... Calculating Kneser-ney Prob for low Order Trigrams = pkn32(t1 t2 t3)...")
               print("...   Pknr(t1 t2 t3) := max{ N1+(t1 t2 t3) - D3, 0 } / ???w N1+(t1 t2 w) + ")
               print("                        D3 / ???w N1+(* t1 t2 w) * N1+(* t1 t2 *) x Pknr (t2 t3) ...")
               
               
               DT.tri.prob[, pkn32:= 
                            ifelse(D3 > n31,0,n31 - D3) /  sum.n32 + 
                            D3 / sum.n32 * n22 * # lambda
                            pkn22,]
               
               
               # Calculate sum.freq4, to be used on Quadgrams Calculations
               print("... Calculating ???w c(t1 t2 t3 w) = sum.freq4(t1 t2 t3) ...")
               
               setkeyv(DT.tri.temp,c("t1","t2","t3"))
               setkeyv(DT.quad.temp,c("t1","t2","t3"))
               
               DT.sum.freq4 <- DT.quad.temp[DT.tri.temp, sum(freq4), by = .EACHI] [,c("t1","t2","t3","sum.freq4","V1"):=list(t1,t2,t3,V1,NULL)]
               DT.tri.prob <- merge(DT.tri.prob, DT.sum.freq4 , by = c("t1","t2","t3"))
             ...
             "4" = {
               ...
               #  Pkn(t1 t2 t3 t4) := max{ c(t1 t2 t3 t4) - D4, 0 } / ???w c(t1 t2 t3 w) + 
               #                             D4 / ???w c(t1 t2 t3 w) * N1+(t1 t2 t3 *) x Pknr (t2 t3 t4)
               
               ##### To calculate this we need 
               #       c(t1 t2 t3 t4) = freq4(t1 t2 t3 t4)    (on Quadgrams Prob Table) 
               #       ???w c(t1 t2 t3 w) = sum.freq4(t1 t2 t3) (on Trigrams Prob Table)
               #       D4                                     (already calculated on init)
               #       N1+(t1 t2 t3 *) = n32(t1 t2 t3)        (on Trigrams Prop Table)
               #       Pknr(t2 t3 t4)  = pkn32(t2 t3 t4)      (on Trigrams Prob Table)
               
               print("Calculating Knersey-ney prob for Quadgrams ...")
               
               
               # Let's add sum.freq4(t1 t2 t3) and n32(t1 t2 t3) to the Quadgrams Table
               
               DT.tri.temp1 <- copy(DT.tri.prob)
               DT.tri.temp1 <- DT.tri.temp1[,list(t1,t2,t3,sum.freq4,n32),]
               DT.quad.prob <- merge(DT.quad.prob, DT.tri.temp1 , by = c("t1","t2","t3"))
               
               # Let's add pkn32(t2 t3 t4) to the Quadgrams Table
               
               DT.tri.temp2 <- copy(DT.tri.prob)
               DT.tri.temp2[,c("t4","t3","t2") := list(t3,t2,t1),] 
               DT.tri.temp2 <- DT.tri.temp2[,list(t2,t3,t4,pkn32),]
               DT.quad.prob <- merge(DT.quad.prob, DT.tri.temp2 , by = c("t2","t3","t4"))
               
               
               # Let's calculate Pkn(t1 t2 t3 t4)
               print("... Calculating Kneser-ney Prob for High Order Quadgrams = pkn41(t1 t2 t3 t4)...")
               
               print("  Pkn(t1 t2 t3 t4) := max{ c(t1 t2 t3 t4) - D4, 0 } / ???w c(t1 t2 t3 w) + ")
               print("                       D4 / ???w c(t1 t2 t3 w) * N1+(t1 t2 t3 *) x Pknr (t2 t3 t4)")
               
               print("   Pkn(t1 t2 t3) := max{ c(t1 t2 t3) - D3, 0 } / ???w c(t1 t2 w) + D3 / ???w c(t1 t2 w) * N1+(t1 t2 *) x Pknr (t2 t3)")
