...
DT_prob_freq <- function (training_set=80) { 
  ...
  for (i in 1:4)
  {
    ...
    DT.result <- rbind(DT.result,as.list(c(i,paste0("Freq <= ",(min_freq+2)),s1+s2,
                                           format_number((s1+s2)/l*100))))
    DT.result <- rbind(DT.result,as.list(c(i,paste0("Freq <= ",(min_freq+3)),s1+s2+s3,
                                           format_number((s1+s2+s3)/l*100))))
    DT.result <- rbind(DT.result,as.list(c(i,paste0("Freq <= ",(min_freq+4)),s1+s2+s3+s4,
                                           format_number((s1+s2+s3+s4)/l*100))))
